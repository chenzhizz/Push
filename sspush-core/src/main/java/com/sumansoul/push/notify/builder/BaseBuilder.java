...
package com.sumansoul.push.notify.builder;
...
import com.sumansoul.push.notify.NotificationUtils;
...
public class BaseBuilder {
    private static final String DEFAULT_CHANNEL_ID_PREFIX = "SSPush_channel_id_";
    private static final String DEFAULT_CHANNEL_NAME_PREFIX = "SSPush_channel_name_";
    ...
    public void build() {
        ...
        mBuilder = new NotificationCompat.Builder(SSPushCore.getContext(), mChannelId);

        if (mSmallIcon > 0) {
            mBuilder.setSmallIcon(mSmallIcon);// 设置顶部状态栏的小图标
        }
        if (mBigIcon > 0) {
            mBuilder.setLargeIcon(BitmapFactory.decodeResource(SSPushCore.getContext().getResources(), mBigIcon));
        }
